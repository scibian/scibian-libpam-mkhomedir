...
/* rajout TMI */
static char *utilisateur;
static char *skelnul = NULL ;
static char localuser[256] = "/local00/home/"  ;
static char localuser1[256] = "/local01/home/"  ;
static char scratch[256] = "/scratch/"  ;
/* fin TMI */
...
if ( pwd->pw_uid < 1000 )
   ...
   if (stat("/local00", &st) == 0 && S_ISDIR(st.st_mode)) {
     strncat(localuser, basename(pwd->pw_dir), 255) ;

     if (stat(localuser, &st) != 0) 
       create_homedir(pwd, skeldir, localuser) ;
   }

  if (stat("/local01", &st) == 0 && S_ISDIR(st.st_mode)) {

   strncat(localuser1, basename(pwd->pw_dir), 255 ) ;

    if (stat(localuser1, &st) != 0)
    create_homedir(pwd, skelnul, localuser1) ;
  ...
  if (stat("/scratch", &st) == 0 && S_ISDIR(st.st_mode)) {
   ...
   strncat(scratch, basename(pwd->pw_dir) , 255 ) ;
    ...
    create_homedir(pwd, skelnul, scratch) ;
}
